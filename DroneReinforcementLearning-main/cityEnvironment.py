...
class GridNavigationEnv(Env):
    ...
    def build_asci_art(self, gridsize, rho):
        ...
        for row in range(1, 26, 8):
          for col in range(1, 31, 8):
            #make the blue square here
            for squareRow in range(row, row+6):
              for squareCol in range(col, col+6):
                art[squareRow][squareCol] = self.OBSTACLE_CHAR


        #green
        art[3][23] = self.GOAL_CHAR
        art[7][7] = self.GOAL_CHAR
        art[18][7] = self.GOAL_CHAR
        art[23][14] = self.GOAL_CHAR
        art[20][24] = self.GOAL_CHAR


        #red
        art[13][15] = self.PLAYER_CHAR
